        ...
        <div class="contentBox">
          ...
          <header>
            <ul class="navList clearfix">
              <!--导航-->
              <li :class="{navActive:navActiveIndex==index}" v-for="item,index in navList" :key="index"
                  ...
                  <li class="clearfix">
                    ...
                    <ul class="roomTypeDetail clearfix">
                      ...
                      <el-checkbox-group v-model="checkedKeYuanData" @change="handleCheckedKeYuanChange">
                        <el-checkbox v-for="keYuan in newKeYuanList" :label="keYuan" :key="keYuan">{{keYuan}}</el-checkbox>
                      </el-checkbox-group>
                    </ul>
                  </li>
                  <li class="clearfix">
                    ...
                    <ul class="roomTypeDetail clearfix">
                      ...
                      <el-checkbox-group v-model="checkedGoRoomWay" @change="handleCheckedGoRoomWayChange">
                        <el-checkbox v-for="goRoomWay in newGoRoomWayList" :label="goRoomWay" :key="goRoomWay">{{goRoomWay}}</el-checkbox>
                      </el-checkbox-group>
                    </ul>
                  </li>
                </ul>
                  ...
                  <li >
                    ...
                    <div>
                      ...
                      <el-checkbox-group v-model="checkedRoomStateData" @change="handleCheckedRoomStateChange">
                        <el-checkbox v-for="roomTai in newRoomTaiList" :label="roomTai" :key="roomTai">{{roomTai}}</el-checkbox>
                      </el-checkbox-group>
                    </div>
                  </li>
                </ul>
    ...
    methods: {
      ...
      changeLouDong(obj,index){
         ...
         * */
          //根据条件楼栋编号查询楼栋数据
        let arr =[];
        arr.push(obj.BuildNo);
        this.conditionSearch('',[],[],[],arr,[]);
        //点击第几栋就显示第几栋的数据
        for(let i=0;i<=this.sectionNavList.length;i++){
          if(i==index){
            this.clickSectionNav(this.sectionNavList[index],index);
          };
        };
        //选择楼栋得楼层数据
        ...
        this.$store.dispatch("homeSearchConditionAction", option)
          .then(
            data => {
              ...
              this.keYuanList.map((item)=>{
                this.newKeYuanList.push(item.wayName+"");
              });
              //入住渠道
              this.goRoomWayList=data.data.custFromList;
              this.goRoomWayList.map((item)=>{
                this.newGoRoomWayList.push(item.memberName+"");
              });
              /**
               * 右侧条件初始化
               * **/
        ...
        this.$store.dispatch("initCashierDeskAction", option)
          .then(
            data => {
              ...
              //条件
              this.InChannelList = data.data.custFromList;
              this.passengerSourceList = data.data.bookWayList;
              //初始化房间
...
//    条件查询
      conditionSearch(hotelID,roomState,custFrom,orderFrom,buildNo,levelNo) {
        let option = {
          "loginUserID": "huileyou",
          "loginUserPass": "123",
          "operateUserID": "",
        ...
        this.$store.dispatch("cashierDeskConditionSearchAction", option)
          .then(
            data => {
              ...
              /*              this.orginRoomStatesList = data.data.hotelConditionList.filter((item)=>{
                              if(item.BuildNo==buildNo[0]){
                                return true;
                              };
                            })[0].LevelList;*/
//              this.roomStatesList=this.orginRoomStatesList;
